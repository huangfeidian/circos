...
namespace spiritsaway::circos
{
	struct Ribbon
	{
		...
		Ribbon(Point center, std::uint16_t radius1, std::uint16_t radius2, amplify_angle span1_begin, amplify_angle span1_end, amplify_angle span2_begin, amplify_angle span2_end, Color in_color, float control_radius_percent = 0, bool twist = false,bool in_fill = false, float in_opacity = 1.0f)
			: color(in_color)
			, fill(in_fill)
			, opacity(in_opacity)

		{
			...
			auto control_point_1 = Point::radius_point(static_cast<std::uint16_t>((radius1 + radius2) / 2 * control_radius_percent), (span1_end + span2_begin) / 2, center);
			auto control_point_2 = Point::radius_point(static_cast<std::uint16_t>((radius1 + radius2) / 2 * control_radius_percent), (span2_end + span1_begin) / 2, center);
